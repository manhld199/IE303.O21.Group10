...
export default function OrderInformationPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    const orderDetails = buyInfo.map((product) => ({
      productIdHashed: product.product_id,
    ...
    const orderDto = {
      ...
      orderShippingCost: 0, // Placeholder, update with actual shipping cost if available
      orderTotalCost: totalWithDiscount,
    };
    ...
    try {
      const response = await axios.post(`${BACKEND_URL}/orders`, orderDto, {
