...
async function getNewsDetail(slug: string, aid: string) {
  try {
    const res = await fetch(`${BACKEND_URL_NEWS}/${slug}/${aid}`, {
      next: { revalidate: 60 },
    });

    console.log("Fetching URL:", `${BACKEND_URL_NEWS}/${slug}/${aid}`);
    ...
    // Assuming the Java API response format is similar to the Node.js API
    return json as IResponseNewsDetail;
  } catch (error) {
...
export async function generateMetadata(
  ...
  const article: IResponseNewsDetail | null = await getNewsDetail(
    newsSlug,
    aid
  );
...
export default async function NewsDetailPage({
  ...
  const newsDetail: IResponseNewsDetail | null = await getNewsDetail(
    newsSlug,
    aid
  );
